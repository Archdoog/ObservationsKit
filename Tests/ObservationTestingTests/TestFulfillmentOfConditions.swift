...
struct TestFillmentOfConditions {
    ...
    func conditionNotMet() async throws {
        ...
        do {
            ...
            #expect(error == .timedOutWith(condition: "ObservationTestingTests/TestFulfillmentOfConditions.swift:28"))
            #expect(error.localizedDescription == "Timed out because condition at ObservationTestingTests/TestFulfillmentOfConditions.swift:28 unmet")
            
            let duration = Date().timeIntervalSince(executionStart)
            #expect(duration.isApproximatelyEqual(to: 1.0, relativeTolerance: 0.2), "timeout is close to expected runtime")
        } catch {
